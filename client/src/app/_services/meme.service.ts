...
getMainMemes(page?: number, itemsPerPage?: number) {
  ...
  getLikes(memeId: number) {
    return this.http.get<Partial<Meme[]>>(this.baseUrl + 'memes/get-user-meme-likes/' + memeId, {});
  }

  getFavourite(memeId: number) {
    return this.http.get<Partial<Meme>>(this.baseUrl + 'memes/get-user-favourite-meme/' + memeId, {});
  }
  ...
  updateMeme(meme: any) {
    return this.http.put(this.baseUrl + 'memes/update-meme/' + meme.id, meme).pipe(
      map(() => {
        const index = this.memes.indexOf(meme);
        this.memes[index] = meme;
      })
    )
  }
  ...
  getMemeTags() {
    return this.http.get<Tag>(this.baseUrl + 'users/get-meme-tags');
  }

  searchMemeTags(searchString: string) {
    return this.http.get<Tag>(this.baseUrl + 'memes/search-memes-tag/' + searchString);
  }

  getMemesByTag(tag: string, page?: number, itemsPerPage?: number) {
    let params = new HttpParams();
    ...
    return this.http.get<Meme[]>(this.baseUrl + 'memes/get-memes-by-tag/' + tag, {observe: 'response', params}).pipe(
      map(response => {
        this.paginatedResult.result = response.body;
        if (response.headers.get('Pagination') !== null) {
          this.paginatedResult.pagination = JSON.parse(response.headers.get('Pagination'));
