...
namespace API.Controllers
{
    ...
    public class MessagesController : BaseApiController
    {
        ...
        [AllowAnonymous]
        public async Task<ActionResult<IEntityTypeConfiguration<MessageDto>>> GetMessages(string username)
        {
            var sender = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername());

            var messages = await _unitOfWork.MessageRepository.GetMessages(username, sender.UserName);
        ...
        public async Task<ActionResult<MessageDto>> SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            return BadRequest("Problem adding message: " + createMessageDto.Content);
        }
